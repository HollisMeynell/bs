...
public interface PoolUserRepository extends JpaRepository<PoolUser, Integer> {
    ...
    @Transactional
    @Query(value = "select u from PoolUser u where u.userId=:userId and u.pool in (select g.pool from PoolCategoryGroup g where g.id=:groupId)")
    Optional<PoolUser> getMapPoolUserByGroupIdAndUserId(int groupId, long userId);
    @Transactional
